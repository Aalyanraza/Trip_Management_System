...
public class Database 
{
    ...
    public UserProfile getUserProfile(int userId) throws SQLException {
        String query = "SELECT FullName, Email, Role FROM Users WHERE UserID = ?";
        try (Connection con = getConnection();
             PreparedStatement stmt = con.prepareStatement(query)) {
            stmt.setInt(1, userId);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return new UserProfile(rs.getString("FullName"), rs.getString("Email"), rs.getString("Role"));
            }
        }
        return null;
    }
	...
	public List<Itinerary> getItineraries(int userId) throws SQLException {
	    String query = "SELECT ItineraryID, TripTitle, TripStartDate, TripEndDate, Destination, Accommodations, Details FROM Itineraries WHERE UserID = ?";
	    List<Itinerary> itineraries = new ArrayList<>();
	    try (Connection con = getConnection();
	         PreparedStatement stmt = con.prepareStatement(query)) {
	        ...
	        while (rs.next()) {
	            Itinerary itinerary = new Itinerary(
	                ...
	                rs.getString("TripStartDate") + " - " + rs.getString("TripEndDate"),  
	                rs.getString("Accommodations"),
    ...
    public void logCollaborator(int itineraryId, int userId, String role) throws SQLException {
        ...
        try (Connection con = getConnection();
            ...
            stmt.setInt(1, itineraryId);
            stmt.setInt(2, userId);
            stmt.setString(3, role);  // Role can be 'Viewer' or 'Editor'
            stmt.executeUpdate();
        }
    }

    public void addTripChat(int itineraryId, int userId, String message) throws SQLException {
        String query = "INSERT INTO TripChat (ItineraryID, UserID, Message) VALUES (?, ?, ?)";
    ...
    public List<String> getTripChat(int itineraryId) throws SQLException {
        String query = "SELECT Message FROM TripChat WHERE ItineraryID = ?";
    ...
    public List<String> getAllFeedbacks() throws SQLException {
        ...
        String sql = "SELECT feedback_message FROM feedbacks";

        try (Connection con = getConnection(); // Use getConnection() here
